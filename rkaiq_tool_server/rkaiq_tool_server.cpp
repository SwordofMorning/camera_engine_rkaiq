...
string g_linuxSocketDomainPath = "/tmp/UNIX.domain3";
...
int main(int argc, char** argv)
{
    ...
    {
        LOG_ERROR("#### ToolServer connect AIQ failed ####\n");
        LOG_ERROR("Failed to connect to domain socket: %s\n", g_linuxSocketDomainPath.c_str());
        
        // 添加更多调试信息
        LOG_DEBUG("Checking if RkAiqServer is running...\n");
        std::string command = "ps -A | grep RkAiqServer";
        int ret = system(command.c_str());
        if (ret == 0) {
            LOG_DEBUG("RkAiqServer process found.\n");
        } else {
            LOG_ERROR("RkAiqServer process not found!\n");
            LOG_ERROR("Please check if RkAiqServer is correctly set up and running.\n");
        }
        
        LOG_DEBUG("Checking domain socket file permission...\n");
        struct stat socket_stat;
        if (stat(g_linuxSocketDomainPath.c_str(), &socket_stat) == 0) {
            LOG_DEBUG("Domain socket file exists.\n");
            LOG_DEBUG("File permission mode: %o\n", socket_stat.st_mode & 0777);
            if ((socket_stat.st_mode & 0777) != 0777) {
                LOG_ERROR("Domain socket file permission incorrect!\n");
                LOG_ERROR("Please check if current user has sufficient privilege.\n");
            }
        } else {
            LOG_ERROR("Domain socket file not found!\n");
            LOG_ERROR("Please check if domain socket path is correct:\n");
            LOG_ERROR("%s\n", g_linuxSocketDomainPath.c_str());
        }
    }
    else
